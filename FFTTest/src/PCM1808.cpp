...
#define AUDIO_BUFFER_SIZE (SAMPLES_MAX*2)

static const char* TAG = "PCM1808";

PCM1808::PCM1808(uint32_t sample_rate) : rx_handle_{nullptr}, sample_rate_{sample_rate}, totalSampleRead_{0}
{
    mutex_ = xSemaphoreCreateMutex();
    if(mutex_ == NULL){
        ESP_LOGE(TAG, "Unable to create mutex");
    }
    buffer = (float*)malloc(SAMPLES_MAX * sizeof(float));
    if(!buffer){
    ...
    if(xSemaphoreTake(mutex_, portMAX_DELAY)){
        ...
        if(buffer){
            for(size_t i=0;i<count;i+=2){
                ...
                buffer[totalSampleRead_++] = static_cast<float>(sample);
...
void PCM1808::initI2S()
{
    ...
    i2s_std_config_t std_cfg = {
        ...
        .slot_cfg = {
            ...
            .slot_bit_width = I2S_SLOT_BIT_WIDTH_32BIT, //The PCM1808 needs a 32 bit slow width (chapt 7.3.5.1.2 of the data sheet "The PCM1808 device accepts 64-BCK/frame or 48-BCK/frame format (only for a 384-fS system clock)" !
            .slot_mode = slot_mode,
            .slot_mask = slot_mask,
            .ws_width = 32,
            .ws_pol = false,
